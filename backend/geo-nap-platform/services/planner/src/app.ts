...
import { plansRouter } from "./api/routes";
...
export function createApp(): express.Express {
  ...
  app.get("/health", (_req, res) => {
    res.status(200).json({ ok: true, service: "planner-service", timestamp: new Date().toISOString() });
  });
  ...
  app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
    logger.error({ error }, "planner-service request failed");
