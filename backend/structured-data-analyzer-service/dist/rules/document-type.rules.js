...
exports.detectDocumentType = void 0;
...
const stripNoise = (text) => text
    ...
    .replace(/[^\w\s:;,.()\-]/g, " ")
    .trim();
const detectDocumentType = (rows) => {
    const matchedSignals = new Set();
    let score = 0;
    for (const row of rows) {
        const rowText = stripNoise((0, keyword_matcher_1.toSearchText)(row));
        if (!rowText) {
            continue;
        }
    ...
    const strongPairsDetected = (matchedSignals.has("pay_as_you_go") && matchedSignals.has("hours_730")) ||
    ...
    const documentType = strongPairsDetected || score >= 4 ? "CLOUD_ESTIMATE" : "REQUIREMENT";
    return {
        documentType,
        score,
