...
const normalizeRawText = (rawText) => rawText
    ...
    .replace(/\n{2,}/g, "\n")
    .trim();
const detailStartRegex = new RegExp(`^(${KNOWN_REGIONS.join("|")})\\s*(?=(?:Standard|Process|\\d+\\s*Customer|Azure VMs|Log analytics|Workloads|Managed Disks|\\d+\\s*[A-Za-z]\\d|\\(Virtual Network))`, "i");
...
const detectServiceFromDescription = (description) => {
...
const parseVmParameters = (description) => {
    const vmMatch = description.match(/(\d+(?:\.\d+)?)\s+([a-z][a-z0-9._\- ]*?)\s*\((\d+(?:\.\d+)?)\s*v(?:cpus?|cores?)\s*,\s*(\d+(?:\.\d+)?)\s*gb\s*ram\)\s*x\s*(\d+(?:\.\d+)?)\s*hours?/i);
    if (!vmMatch) {
        return undefined;
    }
...
const extractCloudEstimateFromParsedInput = (parsed) => {
    ...
    const classifiedServices = entries.map((entry) => {
