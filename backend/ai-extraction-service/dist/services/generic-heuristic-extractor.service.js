...
exports.extractGenericHeuristicCandidate = void 0;
...
const detectRegion = (rawText) => {
    const explicitRegion = rawText.match(/\b(?:region|location)\b\s*[:\-]?\s*([a-z][a-z0-9\- ]{2,40})/i);
    if (explicitRegion?.[1]) {
...
const parseNetworkEgress = (rawText) => {
    const regex = /(\d+(?:\.\d+)?)\s*gb\s*(?:outbound|egress|data\s+transfer(?:\s*out)?|internet\s+egress)/gi;
    let total = 0;
    let matched = false;
...
const parseDatabase = (rawText) => {
    const lower = rawText.toLowerCase();
    const hasDbSignal = lower.includes("database") ||
    ...
    const storageMatch = rawText.match(/(?:database|db|sql)[^.\n]{0,80}?(\d+(?:\.\d+)?)\s*gb/i);
...
const parseFromXmlStructured = (parsed) => {
...
const parseFromText = (rawText) => {
    ...
    const quantityMatch = rawText.match(/(\d+(?:\.\d+)?)\s*(?:instances?|servers?|vms?)\b/i) ??
    ...
    const hasAnySignal = partial.vCPU !== undefined ||
...
const extractGenericHeuristicCandidate = (parsed) => {
    const rawText = parsed.rawText ?? "";
    const region = detectRegion(rawText);
    const compute = parsed.fileType === "xml"
    ...
    if (compute.some((item) => item.vCPU !== undefined && item.ramGB !== undefined)) {
        confidence += 0.15;
    }
    return {
        candidate,
...
exports.extractGenericHeuristicCandidate = extractGenericHeuristicCandidate;
