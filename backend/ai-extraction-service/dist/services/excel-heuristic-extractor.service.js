...
exports.extractExcelHeuristicCandidate = void 0;
...
const getByAliases = (row, aliases, fallbackIndex) => {
...
const extractSheetRows = (parsed) => {
    ...
    for (const block of blocks) {
        ...
        const rows = rowsRaw.filter((row) => Boolean(row && typeof row === "object"));
        normalized.push({ sheetName, rows });
    }
    return normalized;
};
const detectStorageType = (description) => {
...
const parseComputeFromDescription = (description) => {
    const pattern = /(\d+(?:\.\d+)?)\s+([a-z0-9._\-\s]+?)\s*\((\d+(?:\.\d+)?)\s*v(?:cpus?|cores?)\s*,\s*(\d+(?:\.\d+)?)\s*gb\s*ram\)/i;
    ...
    const lower = description.toLowerCase();
    const osType = lower.includes("windows")
        ? "windows"
        : "linux";
    return {
...
const parseEgressGbFromDescription = (description) => {
    const regex = /(\d+(?:\.\d+)?)\s*gb\s*(?:outbound\s+data\s+transfer|outbound|egress|data\s+transfer\s+out)/gi;
    let total = 0;
    let matched = false;
...
const isHeaderRow = (category, serviceType) => {
...
const extractExcelHeuristicCandidate = (parsed) => {
    ...
    const compute = [];
    const computeDedupe = new Set();
    const regionCounts = new Map();
    let networkEgressGb = 0;
    let databaseDetected = false;
    let databaseStorageGb = 0;
    let databaseEngine = "none";
    let databaseHa = false;
    for (const block of sheetRows) {
        for (const row of block.rows) {
            const category = getByAliases(row, ["serviceCategory", "Service category", "Microsoft Azure Estimate"], 0);
            const serviceType = getByAliases(row, ["serviceType", "Service type", "__EMPTY"], 1);
            const regionRaw = getByAliases(row, ["region", "Region", "__EMPTY_2"], 3);
            const description = getByAliases(row, ["description", "Description", "__EMPTY_3"], 4);
            ...
            const computeSignal = parsedCompute &&
            ...
            const isNetworkRow = categoryLower.includes("network") ||
            ...
            const isDatabaseRow = categoryLower.includes("database") ||
...
exports.extractExcelHeuristicCandidate = extractExcelHeuristicCandidate;
