...
export type ReportFormat = "pdf" | "xlsx" | "zip";
...
const toInr = (value: number): string => {
...
const normalizeResults = (results: ProviderCostResult[]): ProviderCostResult[] => {
  if (!Array.isArray(results) || results.length === 0) {
    throw new HttpError(422, "No estimate results available for report generation");
  }
  return results;
};

const renderPdfHeader = (doc: PDFKit.PDFDocument, input: ReportGenerationInput): void => {
...
const renderPdfSummary = (doc: PDFKit.PDFDocument, input: ReportGenerationInput): void => {
...
const renderProviderBreakdown = (
  ...
  result.details.forEach((item) => {
    doc.text(
      `- ${item.serviceType}: ${item.name} | SKU: ${item.sku ?? "-"} | Qty: ${item.quantity} | Monthly: ${toInr(item.monthlyCost)}`
    );
  });
  if (result.optimization?.recommendations?.length) {
    ...
    result.optimization.recommendations.forEach((rec) => {
      doc.text(
        `- [${rec.type}] ${rec.message} | Estimated monthly savings: ${toInr(rec.estimatedMonthlySavings)}`
      );
    });
  }
  doc.moveDown();
...
const renderAssumptions = (
  ...
  doc.text(
    `- Providers included: ${input.results.map((item) => item.provider.toUpperCase()).join(", ")}.`
  );
};

const generatePdfBuffer = async (input: ReportGenerationInput): Promise<Buffer> => {
...
const autoFitColumns = (sheet: ExcelJS.Worksheet): void => {
...
const generateExcelBuffer = async (input: ReportGenerationInput): Promise<Buffer> => {
  ...
  input.results.forEach((row) => {
    const potentialSavings =
      row.optimization?.recommendations?.reduce(
...
export const generateReport = async (
  ...
  const normalizedInput: ReportGenerationInput = {
    ...input,
