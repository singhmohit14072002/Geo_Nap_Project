...
import { AzureRetailPriceItem, getEffectivePrice } from "./azure-retail-pricing.service";
...
export interface AzureEstimateInput {
  region: string;
...
const readFirst = (row: Record<string, unknown>, keys: string[]): string | null => {
...
const parseHours = (description: string): number => {
...
const parseQuantity = (description: string): number => {
  const vmMatch = description.match(
    /(\d+(?:\.\d+)?)\s+[a-z]\d+[a-z0-9._-]*(?:\s*v\d+)?\s*\(/i
  );
  if (vmMatch) {
...
const parseUsageGb = (description: string): number => {
  const regex =
    /(\d+(?:\.\d+)?)\s*gb\s*(?:outbound\s+data\s+transfer|outbound|data\s+transfer|data\s+processed|average\s+monthly\s+backup\s+data)/gi;
  let total = 0;
  let matched = false;
...
const parseCapacityUnits = (description: string): number => {
...
export const normalizeClassifiedAzureServices = (
  input: AzureEstimateInput
): NormalizedAzureService[] => {
  const normalized: NormalizedAzureService[] = [];
  for (const service of input.classifiedServices) {
    const valid = classifiedServiceSchema.safeParse(service);
    if (!valid.success) {
      logger.warn("Skipping invalid classified service row", {
...
const parseNormalizedSkuName = (rawSkuName: string): {
  baseSkuName: string;
  meterName: string | null;
  beginRange: string | null;
  ...
  let beginRange: string | null = null;
...
const mapCloudPricingRowToRetailItem = (row: CloudPricing): AzureRetailPriceItem => {
...
type LocalServicePriceLookup = {
  items: AzureRetailPriceItem[];
  pricingVersion: string | null;
  sourceRegion: string;
};

const loadLocalServicePriceItems = async (
  serviceName: string,
  region: string
): Promise<LocalServicePriceLookup> => {
  try {
    const regionRows = await listLatestCloudPrices("azure", region, serviceName);
    ...
    logger.warn("AZURE_LOCAL_PRICE_REGION_MISS", {
      serviceName,
      requestedRegion: region
    });

    const fallbackRows = await listLatestCloudPricesAnyRegion("azure", serviceName);
    if (fallbackRows.length === 0) {
      logger.warn("AZURE_LOCAL_PRICE_SERVICE_MISS", {
        serviceName,
        requestedRegion: region
      });
      return {
    ...
    logger.warn("AZURE_LOCAL_PRICE_REGION_FALLBACK", {
      serviceName,
      requestedRegion: region,
      fallbackRegion
    });
    ...
    logger.error("AZURE_LOCAL_PRICE_LOOKUP_FAILED", {
      serviceName,
      requestedRegion: region,
...
const toInr = (value: number, currencyCode?: string): number => {
...
const meterText = (item: AzureRetailPriceItem): string =>
  `${item.meterName ?? ""} ${item.productName ?? ""} ${item.armSkuName ?? ""}`.toLowerCase();

const pickCandidateItems = (
  service: NormalizedAzureService,
  items: AzureRetailPriceItem[]
...
const calculateTieredBandwidthCost = (
  usageGB: number,
  items: AzureRetailPriceItem[]
): { monthlyCost: number; unitPriceInr: number | null } => {
  const tiers = items
...
const calculateBackupStorageCost = (
  usageGB: number,
  items: AzureRetailPriceItem[]
): { monthlyCost: number; unitPriceInr: number | null } => {
  ...
  const unit = toInr(getEffectivePrice(chosen), chosen.currencyCode);
  return {
...
const calculateApplicationGatewayCost = (
  service: NormalizedAzureService,
  items: AzureRetailPriceItem[]
): { monthlyCost: number; unitPriceInr: number | null; note: string } => {
  ...
  const gatewayRate = gatewayHourRows[0]
    ? toInr(getEffectivePrice(gatewayHourRows[0]), gatewayHourRows[0].currencyCode)
    : 0;
  const capacityRate = capacityRows[0]
    ? toInr(getEffectivePrice(capacityRows[0]), capacityRows[0].currencyCode)
    : 0;
  ...
  const monthlyCost = round2(
    gatewayRate * hours * quantity + capacityRate * hours * capacityUnits
  );
...
const calculateGenericCost = (
  ...
  const chosen = candidates[0];
  const unitPriceInr = toInr(getEffectivePrice(chosen), chosen.currencyCode);
...
const classificationToDetailType = (
  classification: ClassifiedServiceInput["classification"]
): CostDetailItem["serviceType"] => {
  if (classification === "COMPUTE_VM") {
    return "compute";
  }
  if (classification === "STORAGE_DISK") {
...
const applyBreakdown = (
  classification: ClassifiedServiceInput["classification"],
  amount: number,
  totals: { compute: number; storage: number; database: number; backup: number; network: number; other: number }
) => {
  if (classification === "COMPUTE_VM") {
...
export const estimateAzureCloudEstimatePricing = async (
  ...
  for (const service of normalized) {
    ...
    if (items.length === 0) {
      logger.warn("No local Azure pricing rows for normalized service", {
  ...
  const breakdown = buildBreakdown(
    round2(totals.compute),
    round2(totals.storage),
    round2(totals.database),
    round2(totals.network)
  );
