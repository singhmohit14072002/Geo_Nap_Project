...
const parseJsonEnv = <T>(raw: string | undefined): T | null => {
...
const createGcpBillingClient = async (): Promise<CloudCatalogClient> => {
  const inlineCredentials = parseJsonEnv<Record<string, unknown>>(
  ...
  return new CloudCatalogClient(options as ConstructorParameters<typeof CloudCatalogClient>[0]);
};

const parsePriceFromSku = (sku: Record<string, unknown>): ParsedSkuPrice | null => {
  const pricingInfos = (sku.pricingInfo as Array<Record<string, unknown>> | undefined) ?? [];
  if (pricingInfos.length === 0) {
    return null;
  }

  let best: ParsedSkuPrice | null = null;
  for (const info of pricingInfos) {
    const expression = (info.pricingExpression as Record<string, unknown> | undefined) ?? {};
    const usageUnit = String(expression.usageUnit ?? "unit");
    const tieredRates =
      (expression.tieredRates as Array<Record<string, unknown>> | undefined) ?? [];

    for (const tier of tieredRates) {
      const startUsageAmount = Number(tier.startUsageAmount ?? 0);
      const unitPrice = (tier.unitPrice as Record<string, unknown> | undefined) ?? {};
      ...
      const candidate: ParsedSkuPrice = {
        usd,
...
const filterSkusByRegion = (
  skus: Array<Record<string, unknown>>,
  region: string
): Array<Record<string, unknown>> => {
  return skus.filter((sku) => {
    const serviceRegions = (sku.serviceRegions as string[] | undefined) ?? [];
    if (serviceRegions.includes(region)) {
      return true;
    }
    const geoTaxonomy = (sku.geoTaxonomy as Record<string, unknown> | undefined) ?? {};
    const regions = (geoTaxonomy.regions as string[] | undefined) ?? [];
...
const pickCoreSku = (skus: Array<Record<string, unknown>>): SelectedSku | null => {
  const candidates: SelectedSku[] = [];
  for (const sku of skus) {
    const category = (sku.category as Record<string, unknown> | undefined) ?? {};
...
const pickRamSku = (skus: Array<Record<string, unknown>>): SelectedSku | null => {
  const candidates: SelectedSku[] = [];
  for (const sku of skus) {
    const category = (sku.category as Record<string, unknown> | undefined) ?? {};
    const usageType = String(category.usageType ?? "").toLowerCase();
...
const pickDiskSku = (skus: Array<Record<string, unknown>>): SelectedSku | null => {
  const candidates: SelectedSku[] = [];
  for (const sku of skus) {
    const category = (sku.category as Record<string, unknown> | undefined) ?? {};
    const usageType = String(category.usageType ?? "").toLowerCase();
...
const pickEgressSku = (skus: Array<Record<string, unknown>>): SelectedSku | null => {
  const candidates: SelectedSku[] = [];
  for (const sku of skus) {
    const category = (sku.category as Record<string, unknown> | undefined) ?? {};
    const usageType = String(category.usageType ?? "").toLowerCase();
...
const loadComputeEngineSkus = async (
  client: CloudCatalogClient
): Promise<Array<Record<string, unknown>>> => {
  const servicesRequest = {};
  let computeServiceName: string | null = null;

  for await (const service of client.listServicesAsync(servicesRequest)) {
    const displayName = String((service as Record<string, unknown>).displayName ?? "");
    if (displayName.toLowerCase() === "compute engine") {
      computeServiceName = String((service as Record<string, unknown>).name ?? "");
      break;
    }
  }
  ...
  for await (const sku of client.listSkusAsync({ parent })) {
    skus.push(sku as Record<string, unknown>);
  }
  return skus;
};

export const fetchAndNormalizeGcpPricingRows = async (
  rawRegion: string,
