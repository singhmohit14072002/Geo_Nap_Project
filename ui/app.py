...
with st.sidebar:
    ...
    high_contrast = st.toggle("High contrast text", value=True)
...
# Visual Theme
# -----------------------------
ink = "#14110d" if high_contrast else "#1f1d1a"
muted = "#4a3f36" if high_contrast else "#6d645a"
hero_ink = "#1c140e" if high_contrast else "#2b2118"
hero_muted = "#3c2f24" if high_contrast else "#5a4a3a"
border = "#e0d3c0" if high_contrast else "#eadfce"
...
# Cache Safety Check
# -----------------------------
providers_path = Path("cache/providers.json")
...
with st.form("plan_form"):
    ...
    with dc1:
        data_source_provider = st.text_input("Data source provider (e.g., aws)", value="aws")
    with dc2:
...
for line in override_text.splitlines():
    ...
    if len(parts) == 2:
        try:
            egress_overrides[parts[0]] = float(parts[1])
        except ValueError:
            pass
