...
import { ExtractionLog } from "../models/extraction-log.model";

const sendResponse = async (res: Response, payload: any) => {
  try {
    await ExtractionLog.create(payload);
  } catch (err) {
    logger.error("Failed to save extraction log to MongoDB", { error: err });
  }
