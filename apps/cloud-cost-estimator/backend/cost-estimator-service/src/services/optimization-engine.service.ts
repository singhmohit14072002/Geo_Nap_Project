...
const buildRightSizingRecommendation = (
  ...
  return {
    type: "RIGHT_SIZING",
    message:
      `Detected over-provisioning (CPU gap ${(cpuGap * 100).toFixed(0)}%, RAM gap ${(ramGap * 100).toFixed(0)}%). ` +
      "Use a smaller VM family/size to reduce unused compute capacity.",
    estimatedMonthlySavings
  };
};

const buildReservedInstanceRecommendations = (
  result: ProviderCostResult
): OptimizationRecommendation[] => {
  const computeRows = result.details.filter((item) => item.serviceType === "compute");
  ...
  const hoursPerMonth =
    numberMeta(computeRows[0].metadata, "hoursPerMonth") ?? 730;
  ...
  return [
    {
      ...
      message:
        "Workload appears to run continuously (24x7). Consider 1-year reserved capacity to reduce compute spend.",
      estimatedMonthlySavings: round2(
        result.breakdown.compute * RESERVED_1Y_SAVINGS_RATE
      )
    },
    {
      type: "RESERVED_INSTANCE",
      message:
        "For long-lived workloads, 3-year reserved capacity provides deeper discounts than pay-as-you-go.",
      estimatedMonthlySavings: round2(
        result.breakdown.compute * RESERVED_3Y_SAVINGS_RATE
      )
    }
  ];
};

const buildStorageRecommendation = (
  result: ProviderCostResult
): OptimizationRecommendation | null => {
  ...
  const hasPremium = storageRows.some((row) => {
    ...
    const tier =
      typeof metaTier === "string"
        ? metaTier.toLowerCase()
        : extractStorageTier(row.sku) ?? extractStorageTier(row.name);
    return tier === "premium";
  });

  const highIopsRequired = storageRows.some(
    (row) => boolMeta(row.metadata, "highIopsRequired") === true
  );
  ...
  return {
    ...
    message:
      "Premium storage is detected without a high-IOPS requirement. Downgrade to a standard tier for lower cost.",
    estimatedMonthlySavings: round2(result.breakdown.storage * 0.2)
  };
};

const buildNetworkRecommendation = (
  result: ProviderCostResult
): OptimizationRecommendation | null => {
  const networkRow = result.details.find(
    (item) => item.serviceType === "network-egress"
  );
  ...
  return {
    ...
    message:
      `Data egress is high (${dataEgressGb.toFixed(0)} GB). Consider co-locating data and compute, CDN/cache layers, or reducing cross-region transfer.`,
    estimatedMonthlySavings: round2(result.breakdown.networkEgress * 0.15)
  };
};

const buildProviderRecommendations = (
  result: ProviderCostResult
): ProviderOptimizationRecommendations => {
...
export const attachOptimizationRecommendations = (
  results: ProviderCostResult[]
): ProviderCostResult[] => {
  const recommendations = generateOptimizationRecommendations(results);
  const byProvider = new Map(
    recommendations.map((item) => [item.provider, item])
  );
