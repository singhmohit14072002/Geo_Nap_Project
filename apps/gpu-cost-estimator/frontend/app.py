...
# Add project root to Python path so common files in nexus can be loaded
APP_ROOT = Path(__file__).resolve().parents[2] / "cloud-cost-nexus"
sys.path.insert(0, str(APP_ROOT))

# We need to also add ui folder if cost details etc requires it
UI_ROOT = APP_ROOT / "ui"
...
def build_report_html(base_result, model_result):
    html = ["<h2>Cloud Cost Nexus Report</h2>"]
...
st.markdown("Define requirements, then let Cloud Cost Nexus choose the best placement across providers and regions.")
...
if run_clicked:
    ...
    with st.spinner("Running Cloud Cost Nexus optimization..."):
